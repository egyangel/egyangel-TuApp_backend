...
[Get Postman Collection](http://localhost/apidocs/collection.json)
...
```
```javascript
const url = new URL("/api/users/register");
...
```
```javascript
const url = new URL("/api/users/login");
...
```bash
curl -X POST "/api/users/logout" 
```
```javascript
const url = new URL("/api/users/logout");
